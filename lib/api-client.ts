...
//   private tenantSlug: string | null = null; // 👈 add
...
//       this.tenantSlug = localStorage.getItem('tenant_slug'); // 👈 restore tenant
//     }
//   }

//   // ---- add helpers for tenant ----
...
//     if (this.tenantSlug) headers['X-Tenant-Id'] = this.tenantSlug; // 👈 attach tenant on every call
...
export class ApiClient {
  ...
  setTenant(slug: string) {
    this.tenantSlug = slug;
    if (typeof window !== 'undefined') localStorage.setItem('tenant_slug', slug);
  }
  clearTenant() {
    this.tenantSlug = null;
    if (typeof window !== 'undefined') localStorage.removeItem('tenant_slug');
  }
  getTenant() {
    return this.tenantSlug;
  }
  ...
  // ---------- core request ----------
  private async request<T>(endpoint: string, method: HttpMethod = 'GET', body?: unknown, init?: RequestInit): Promise<T> {
    const url = `${this.baseUrl}${endpoint}`;
    ...
    if (this.tenantSlug) headers['X-Tenant-Id'] = this.tenantSlug;
    if (this.token) headers['Authorization'] = `Bearer ${this.token}`;
  ...
  async login(credentials: LoginCredentials): Promise<AuthResponse> {
    // POST /api/auth/login  (X-Tenant-Id required)
    const resp = await this.request<AuthResponse>('/auth/login', 'POST', credentials);

    // persist token & tenant
    this.setToken(resp.token);
    if (resp.organization?.slug) this.setTenant(resp.organization.slug);
  ...
  async logout(): Promise<void> {
    try {
      await this.request('/auth/logout', 'POST'); // server is stateless; ok if not used
    } catch { /* ignore */ }
    this.clearToken();
    this.clearTenant();
  }
  ...
  getCustomerEngagement(): Promise<CustomerEngagement> {
    return this.request('/dashboard/customer-engagement', 'GET');
  }
  getProductAnalytics(): Promise<ProductAnalytics> {
    return this.request('/dashboard/product-analytics', 'GET');
  }
  getCustomerAnalytics(): Promise<CustomerAnalytics> {
    return this.request('/dashboard/customer-analytics', 'GET');
  }
  ...
  // Public list (your backend has a public GET /products)
  getProducts(params?: { page?: number; limit?: number; category?: string; search?: string }): Promise<{
    products: Product[];
    total: number;
    pages: number;
  }> {
    return this.request(`/products${buildQuery(params)}`, 'GET');
  }
  ...
  getCategory(id: string): Promise<Category> {
    return this.request(`/categories/${id}`, 'GET');
  }
  createCategory(input: { name: string; slug: string; image?: string | null }): Promise<Category> {
    return this.request('/categories', 'POST', input);
  }
  updateCategory(id: string, input: Partial<Category>): Promise<Category> {
    return this.request(`/categories/${id}`, 'PUT', input);
  }
  ...
  getUsers(params?: { page?: number; limit?: number; search?: string }): Promise<{ users: User[]; total: number; pages: number; }> {
    return this.request(`/users/${buildQuery(params)}`, 'GET');
  }
  getUser(id: string): Promise<User> {
    return this.request(`/users/${id}`, 'GET');
  }
  createUser(input: {
    name: string; email: string; phone?: string; password: string; is_verified?: boolean;
  }): Promise<User> {
    return this.request('/users/', 'POST', input);
  }
  updateUser(id: string, input: Partial<User>): Promise<User> {
    return this.request(`/users/${id}`, 'PUT', input);
  }
  ...
  getActiveOffers(): Promise<Offer[]> {
    return this.request('/offers/active', 'GET');
  }
  // Protected
  getOffers(): Promise<Offer[]> {
    return this.request('/offers', 'GET');
  }
  getOffer(id: string): Promise<Offer> {
    return this.request(`/offers/${id}`, 'GET');
  }
  createOffer(input: CreateOfferRequest): Promise<Offer> {
    return this.request('/offers', 'POST', input);
  }
  updateOffer(id: string, input: Partial<Offer>): Promise<Offer> {
    return this.request(`/offers/${id}`, 'PUT', input);
  }
  deleteOffer(id: string): Promise<{ message?: string }> {
    return this.request(`/offers/${id}`, 'DELETE');
  }
  ...
  getActiveBanners(): Promise<Banner[]> {
    return this.request('/banners/active', 'GET');
  }
  // Protected
  getBanners(): Promise<Banner[]> {
    return this.request('/banners', 'GET');
  }
  getBanner(id: string): Promise<Banner> {
    return this.request(`/banners/${id}`, 'GET');
  }
  createBanner(input: CreateBannerRequest): Promise<Banner> {
    return this.request('/banners', 'POST', input);
  }
  updateBanner(id: string, input: Partial<Banner>): Promise<Banner> {
    return this.request(`/banners/${id}`, 'PUT', input);
  }
  deleteBanner(id: string): Promise<{ message?: string }> {
    return this.request(`/banners/${id}`, 'DELETE');
  }
}
